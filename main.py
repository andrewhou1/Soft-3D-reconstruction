...
def main():
	...
	(neighbor_param_files, neighbor_img_files) = get_neighbors(camera_parameter_files, img_files, int(sys.argv[2]), 5)
	...
	for i in range(len(neighbor_param_files)):
		(threeD_pos1, orientation_matrix1, focal_len1, principal_point1) = read_camera_parameters(img_and_parameter_dir+camera_parameter_files[int(sys.argv[2])])	
		...
		img1 = cv2.imread(img_and_parameter_dir+img_files[int(sys.argv[2])])
		img2 = cv2.imread(img_and_parameter_dir+neighbor_img_files[i])
		(img1_rect, img2_rect, horizontally_rectified) = rectify(threeD_pos1, threeD_pos2, orientation_matrix1, orientation_matrix2, focal_len1, focal_len2, principal_point1, principal_point2, img_height, img_width, img1, img2)		
	...
	cumulative_SADs = cumulative_SAD(current_img_rectified, rectified_neighbors, current_camera_focal_len, neighbor_focal_lens, current_3D_pose, neighbor_3D_poses, horizontal_rec)
	...
	cv2.imwrite("out.png", estimated_depth_map)
